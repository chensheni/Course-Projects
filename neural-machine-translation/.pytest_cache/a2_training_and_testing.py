...
def train_for_epoch(
    ...
    # the loop.
    loss_function = torch.nn.CrossEntropyLoss(ignore_index = model.source_pad_id)
    ...
    for source_x, source_x_lens, target_y in tqdm(dataloader):
        ...
        #print(f'source_x_lens:{source_x_lens.shape}')
        #print(f'target_y:{target_y.shape}')
        #print(f'source_x:{source_x[:,0]}')
        #print(f'source_x_lens:{source_x_lens[0]}')
        #print(f'target_y:{target_y[:,0]}')
        ...
        # determine next-token probabilities
        logits = model(source_x, source_x_lens, target_y).to(device)

        # getting rid of a token and replacing excess end-of-sequence tokens with padding
        target_y = torch.masked_fill(target_y, 
                                    model.get_target_padding_mask(target_y), 
                                    model.source_pad_id)
        # delete the first SOS tag
        target_y = target_y[1:]

        #print(f'logits:{logits.shape}')
        #print(f'target_y:{target_y.shape}')

        # flattens out the sequence dimension into the batch dimension
        # to fit in the loss function
        logits   = logits.reshape(logits.shape[0] * logits.shape[1], logits.shape[2]).to(device)
        target_y = target_y.reshape(target_y.shape[0] * target_y.shape[1]).to(device)
        #print(f'logits:{logits.shape}')
        #print(f'target_y:{target_y.shape}')
...
def compute_batch_total_bleu(
    ...
    total_bleu : float
        ...
        n-gram precision 4.
    '''
    # you can use target_y_ref.tolist() to convert the LongTensor to a python list of numbers
    total_bleu = 0

    # batch size
    batch_size = target_y_ref.shape[1]
    
    # loop over candidates and references
    for i in range(batch_size):
        # convert to list
        reference = target_y_ref[:, i].tolist()
        candidate = target_y_cand[:, i].tolist()
        # delete sos and eos
        reference = [i for i in reference if ((i != target_sos) and (i != target_eos))]
        candidate = [i for i in candidate if ((i != target_sos) and (i != target_eos))]
        # calculation
...
def compute_average_bleu_over_dataset(
    ...
    # loop over triples in dataloader
