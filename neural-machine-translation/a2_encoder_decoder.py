...
class Encoder(EncoderBase):
    ...
    def get_all_hidden_states(
        ...
        # print(f'x:{x}')
        x_pack = torch.nn.utils.rnn.pack_padded_sequence(input = x, lengths = source_x_lens.cpu(), enforce_sorted = False)
...
class DecoderWithoutAttention(DecoderBase):
    ...
    def get_first_hidden_state(
        ...
        # forward & backward (before padding)
        slice_array = torch.arange(source_x_lens.shape[0])
        forward  = h[source_x_lens - 1, slice_array , 0 : self.hidden_state_size//2]
    ...
    def get_current_rnn_input(
        ...
        xtilde_t = self.embedding(target_y_tm1).to(h.device)
        return xtilde_t
    ...
    def get_current_hidden_state(
        ...
        # unsqueeze for model implementation
        if self.cell_type == 'lstm':
...
class DecoderWithMultiHeadAttention(DecoderWithAttention):
    ...
    def attend(
        ...
        # 8. transform the concatenated multi-head c
        output = self.Q(ctn)
        return output
...
class EncoderDecoder(EncoderDecoderBase):
    ...
    def translate(self, input_sentence):
        ...
        for i in range(token_cnt):
            # check if in the self.dataset.source_word2id
    ...
    def update_beam(
        ...
        # update paths and reshape to B, KV
        logpb_tm1_repeat = logpb_tm1.unsqueeze(-1).repeat(1, 1, self.target_vocab_size)
        extended_path = logpb_tm1_repeat + logpy_t
        extended_path = extended_path.view(logpy_t.shape[0], -1)

        # select path
        logpb_t, index = torch.topk(extended_path, k = self.beam_width, dim = 1)
        #print(f'logpb_t{logpb_t.shape}')
        #print(f'index{index.shape}')
        paths = torch.div(index, self.target_vocab_size,  rounding_mode = 'floor')
        #print(f'paths{paths}')
        index = torch.remainder(index, self.target_vocab_size)
        #print(f'index{index}')

        # transformation
        paths_unsqueeze = paths.unsqueeze(dim = 2)
        index_unsqueeze = index.unsqueeze(dim = 0)

        # k-th path hidden state
        if   self.decoder.cell_type == "lstm":
            b_t_0 = (torch.gather(htilde_t[0], 1, paths_unsqueeze.expand_as(htilde_t[0])),
                     torch.gather(htilde_t[1], 1, paths_unsqueeze.expand_as(htilde_t[1])))
        elif self.decoder.cell_type == "rnn":
            b_t_0 = torch.gather(htilde_t, 1, paths_unsqueeze.expand_as(htilde_t))

        # k-th path sequence
        b_tm1_1 = torch.gather(b_tm1_1, 2, paths.expand_as(b_tm1_1))
        b_t_1 = torch.cat([b_tm1_1, index_unsqueeze], dim = 0)
