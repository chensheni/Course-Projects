...
def grouper(seq:Sequence[str], n:int) -> List:
    ...
    ngrams : list
    '''
    # count have many should be in ngrams
...
def n_gram_precision(reference:Sequence[str], candidate:Sequence[str], n:int) -> float:
    ...
    # initialize for all cases (including len(can) == 0)
    p_n = 0
...
def brevity_penalty(reference:Sequence[str], candidate:Sequence[str]) -> float:
    ...
    BP : float
        ...
        of 0 length, `BP` is 0.
    '''
    # In the case that the candidate transcription is of 0 length, `BP` is 0
    if len(candidate) == 0:
        return 0

    brevity = float(len(reference)) / float(len(candidate))
    
    # case of ri < ci and ri >= ci
    if brevity < 1: 
        return 1
    else:        
